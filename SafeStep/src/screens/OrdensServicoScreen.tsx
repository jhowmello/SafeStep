...
export default function OrdensServicoScreen({ navigation }: Props) {
  ...
  const fetchOrdens = useCallback(async (isRefresh = false) => {
    ...
    try {
      const resposta = await fetch(`${BASE_URL}/ordensServico`);
  ...
  async function confirmarDelete(id: number) {
    try {
      const resposta = await fetch(`${BASE_URL}/ordensServico/${id}`, {
        method: 'DELETE',
      });
  ...
  function handleDelete(id: number) {
    ...
    Alert.alert(
      ...
      [
        ...
        { text: 'Excluir', style: 'destructive', onPress: () => confirmarDelete(id) },
      ]
    );
  }
